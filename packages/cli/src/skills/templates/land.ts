...
export function generateLandSkill(_ctx: SkillTemplateContext): string {
  ...
  lines.push(
    "4. **Trivial conflict** — when a merge conflict is confined to `.changeset/*`, `docs/**`, `CHANGELOG.md`, or the repository lockfile, resolve it in the current Land cycle through the pull skill; keep both changesets and regenerate the lockfile with the repository package manager when needed. Commit, publish through the push skill, and re-run the merged guard plus every pre-flight check while remaining in `Land`. No other file may be edited."
  );
  lines.push(
    "5. **Rework failure** — failed required CI, a source-file merge conflict (`mergeable: CONFLICTING` outside the trivial set defined in item 4), a missing required changeset, or an unresolved actionable review thread created after the latest qualifying human approval on the current head (compare its first comment's `createdAt` with the approval's `submittedAt`): prepare a status body with reason `Land-return rework: <cause>`, send `Land` → `Ready` transition intent through the gh-project skill, then publish the body through `github_graphql` after confirmed readback. An unresolved actionable thread created at or before that approval is absorbed by the approval and does not block Land. The Ready-return rework guard opens the next work cycle and routes the item to `In progress`."
  );
  lines.push(
