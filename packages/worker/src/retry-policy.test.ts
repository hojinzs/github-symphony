...
describe("calculateRetryDelay", () => {
  it("uses exponential backoff", () => {
    expect(
      calculateRetryDelay(1, { baseDelayMs: 1000, maxDelayMs: 30000 })
    ).toBe(1000);
    expect(
      calculateRetryDelay(2, { baseDelayMs: 1000, maxDelayMs: 30000 })
    ).toBe(2000);
    expect(
      calculateRetryDelay(3, { baseDelayMs: 1000, maxDelayMs: 30000 })
    ).toBe(4000);
  });

  it("caps the retry delay at the configured maximum", () => {
    expect(
      calculateRetryDelay(8, { baseDelayMs: 1000, maxDelayMs: 5000 })
    ).toBe(5000);
  });
});
