...
import { launchCodexAppServer, prepareCodexRuntimePlan, type CodexRuntimeConfig } from "./runtime.js";
...
export function resolveLocalRuntimeLaunchConfig(
  ...
  if (!workspaceId) {
    throw new LocalRuntimeLauncherError(
      "WORKSPACE_ID or CODEX_WORKSPACE_ID is required."
    );
  }
...
export function loadLauncherEnvironment(
  ...
  const mergedEnv = {
    ...readEnvFile(resolve(dirname(fileURLToPath(import.meta.url)), "..", ".env")),
...
function emitLaunchSummary(config: CodexRuntimeConfig) {
  ...
  process.stdout.write(
    [
      ...
      `[worker] workspace: ${config.workspaceId}`,
