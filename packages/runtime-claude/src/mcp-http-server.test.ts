...
describe("Claude host MCP HTTP server", () => {
  ...
  it("uses Streamable HTTP semantics and keeps GitHub available for Linear", async () => {
    server = await startClaudeMcpHttpServer({
      ...
      context: { issue: { id: "issue-1", identifier: "owner/repo#1", nativeRef: {} } },
    });
    const headers = { authorization: `Bearer ${server.sessionToken}`, "content-type": "application/json" };
    const initialize = await fetch(server.url, { method: "POST", headers, body: JSON.stringify({ jsonrpc: "2.0", id: 1, method: "initialize" }) });
    expect(initialize.status).toBe(200);
    const notification = await fetch(server.url, { method: "POST", headers, body: JSON.stringify({ jsonrpc: "2.0", method: "notifications/initialized" }) });
    expect(notification.status).toBe(202);
    const tools = await fetch(server.url, { method: "POST", headers, body: JSON.stringify({ jsonrpc: "2.0", id: 2, method: "tools/list" }) });
    await expect(tools.json()).resolves.toMatchObject({ result: { tools: expect.arrayContaining([expect.objectContaining({ name: "github_graphql" }), expect.objectContaining({ name: "linear_graphql" })] } });
