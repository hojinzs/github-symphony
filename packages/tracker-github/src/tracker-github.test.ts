...
Prompt`,
  ...
  it("does not expose orchestrator-owned comment writers", () => {
    const adapter = resolveTrackerAdapter({
      adapter: "github-project",
      bindingId: "project-123",
    });

    expect(adapter).not.toHaveProperty("upsertTransitionComment");
    expect(adapter).not.toHaveProperty("upsertIssueComment");
  });
