...
Project start also holds a host-local lock keyed by the canonical project-folder
path. Consequently, pointing separate `--config` runtime roots at the same
project folder cannot start duplicate orchestrators. A dead or identity-mismatched
owner is reclaimed using the same lease and process-identity checks as the
per-runtime-root project lock.
