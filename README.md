...
Linear workflows may also configure `tracker.pickup_labels.include` and `tracker.pickup_labels.exclude` as pickup eligibility gates. Excluded labels always win; when include labels are configured, an issue needs at least one include label before a worker starts. Label comparison is case-insensitive and ignores surrounding whitespace, so labels that differ only by case or outer whitespace cannot be used as separate gates. A label change that makes a running issue non-dispatchable is observed during reconciliation and stops that worker; use lifecycle state and handoff policy to make the intended outcome explicit.
