...
- **THEN** the control plane provisions a dedicated Symphony runtime mapped to that workspace by using the configured runtime driver
...
- **THEN** the control plane creates the GitHub Project and dedicated Symphony runtime for that workspace by using the configured runtime driver

#### Scenario: Local runtime driver in development
- **WHEN** the operator submits a valid workspace creation request while the control plane is configured for the local runtime driver
- **THEN** the control plane provisions the workspace runtime without creating a Docker container
- **THEN** the created runtime instance points to a host-local worker process and reachable state endpoint for that workspace
...
- **THEN** no GitHub Project or Symphony runtime is created
...
The system SHALL persist the identifiers and status needed to manage each workspace runtime, including the workspace configuration, linked repositories, GitHub Project identifiers, effective agent credential source, runtime driver, generic runtime identity, endpoint host and port, workflow path, optional local-process metadata, and last known lifecycle state.

#### Scenario: Runtime status lookup
- **WHEN** the control plane loads a workspace detail or dashboard view
- **THEN** it can resolve the workspace configuration, effective agent credential source, runtime driver, and mapped runtime instance from persisted metadata
...
The system SHALL provide a dashboard view that summarizes the live state of all active workspaces by combining persisted metadata with each worker runtime state endpoint.

#### Scenario: Dashboard state aggregation
- **WHEN** a user opens the workspace dashboard
- **THEN** the control plane queries all active workspace state endpoints in parallel regardless of runtime driver
- **THEN** the response identifies each workspace, its runtime driver, runtime health, and its currently observed execution state
