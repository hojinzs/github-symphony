...
| Codex host-side dynamic tools                                                 | `packages/runtime-codex/src/runtime.test.ts`, `packages/worker/src/worker-protocol.test.ts`, and `packages/worker/src/codex-dynamic-tools.test.ts` cover advertised schemas, `item/tool/call` responses, structured failures, and issue-context forwarding.                                                                                                                                                                                                                                                                 | `docker compose -f docker-compose.e2e.yml exec -T symphony-e2e node /app/e2e/host-dynamic-tool-e2e.mjs` runs the built worker helper against a Docker-contained provider stub and verifies one host-side call plus its issue context. |
