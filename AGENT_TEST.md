...
| Hook-owned repository population and configuration faults                   | `packages/cli/src/workflow/default-hooks.test.ts` verifies `workflow init` writes an executable population hook and executes it against a real bare fixture; `packages/cli/src/commands/{start,doctor}.test.ts` cover absent and non-executable paths; `packages/orchestrator/src/service.test.ts` proves a missing hook degrades the project without changing an issue retry count.                                                                                                                                                                                                                                                                 | `./e2e/run-standalone-project-e2e.sh` first rejects a packaged standalone project with a missing hook and names its absolute path, then dispatches two valid projects whose provisioned `after_create` hooks populate workspaces before the stub workers start.                                                                                                                                                                                                                                                                                                                                                  |
