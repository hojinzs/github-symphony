...
| Dirty-workspace recovery warning                                             | `packages/orchestrator/src/service.test.ts` verifies a dirty recovery workspace remains in place, recovery context reaches the worker, no `.quarantine-*` directory is created, and a `recovery-dirty-workspace` event records the visible dirty files; `packages/core/src/observability/event-formatter.test.ts` verifies the operator-facing warning.                                                                                                                                                                                                                                                         | `STUB_SCENARIO=recovery-fail` exercises dirty recovery redispatch in Docker; inspect `events.ndjson` for `recovery-dirty-workspace` and confirm the workspace path is unchanged between attempts.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| Agent child isolation and host Git transport                                 | `packages/runtime-codex/src/runtime.test.ts`, `packages/runtime-claude/src/{adapter,mcp-compose}.test.ts`, `packages/core/src/runtime/custom-child-env.test.ts`, `packages/worker/src/non-codex-runtime.test.ts`, `packages/tracker-{github,linear}` suites, and `packages/worker/src/git-transport.test.ts` cover unconditional raw/broker-secret stripping, isolated HOME/GH config, disabled subprocess MCP, custom-auth forwarding, compatibility mode, and real bare-remote fetch/push.                                                                                                                    | [TC-20](e2e/scenarios/20-agent-child-isolation.md) runs `pnpm e2e:claude`; a real custom child covers both default isolation and the documented compatibility mode, proving its private HOME/GH config and Git-helper boundary while differentiating raw-secret exposure. The Claude and Codex children dump their complete environments so the black-box assertion can inject an independent literal credential list, cross-check it with the exported contract, assert literal host-constructed paths and visible context, and reject any leaked sentinel. Host tool mutations and the post-run assigned-branch push still succeed. Real Claude and Codex worker lifecycles also exercise a non-fast-forward transport failure and assert a failed terminal heartbeat plus non-zero process exit. |
| Tenant-scoped worker tracker credentials                                     | `packages/orchestrator/src/service.test.ts` verifies daemon credential injection, project `.env` precedence, missing-credential dispatch suppression/status warnings, and continued rejection of unrelated daemon keys; `packages/worker/src/tracker-credential-preflight.test.ts` covers GitHub direct/broker and Linear credentials; `packages/cli/src/commands/doctor.test.ts` covers the worker-effective GitHub check. Tracker adapter suites verify provider-specific selection, while runtime isolation and Git transport suites prove the selected secret remains host-only.                            | [TC-20](e2e/scenarios/20-agent-child-isolation.md) runs the built worker without a GitHub credential and confirms a terminal startup failure occurs before runtime initialization; its credential-bearing path still confirms host tracker tools and post-run Git push succeed while the child receives no tracker credential or inherited Git helper.                                                                                                                                                                                                                                                                                                                                                                                                                                              |
