...
| Clone populate template branch assignment                                    | `packages/orchestrator/src/git.test.ts` verifies a rendered template branch starts at `origin/<base_branch>` without an upstream, a linked PR head wins, and an existing template checkout is reused untouched; `packages/orchestrator/src/service.test.ts` verifies dispatch rejects a configured template left on its base branch.                                                                                                                                                                                                                                                                                                                         | `./e2e/run-e2e.sh happy 60` confirms the packaged repo-init → clone-populate → worker dispatch lifecycle uses the assigned issue branch.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
