...
| Tenant-scoped worker tracker credentials                                     | `packages/orchestrator/src/service.test.ts` verifies daemon credential injection, project `.env` precedence, missing-credential dispatch suppression/status warnings, and continued rejection of unrelated daemon keys; `packages/worker/src/tracker-credential-preflight.test.ts` requires the direct GitHub tracker token while retaining Linear alternatives; `packages/cli/src/commands/doctor.test.ts` covers the worker-effective GitHub check. The GitHub adapter and tool suites prove polling and host tool calls use the declared direct token, while runtime isolation and Git transport suites prove the selected secret remains host-only and the separate publication broker path is unchanged. | [TC-19](e2e/scenarios/19-host-side-tracker-tools.md) executes host-owned GitHub queries and mutations with a direct token at the HTTP boundary. [TC-20](e2e/scenarios/20-agent-child-isolation.md) runs the built worker without a GitHub credential and confirms a terminal startup failure occurs before runtime initialization; its credential-bearing path still confirms host tracker tools and post-run Git push succeed while the child receives no tracker credential or inherited Git helper.                                                                                                                                                                                                                                                                                              |
