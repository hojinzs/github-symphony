...
| Dirty unpublished worktree after host transport                              | `packages/worker/src/git-transport.test.ts` verifies the host push returns tracked and untracked unpublished file lists without classifying the successful push as a transport failure. `packages/orchestrator/src/service.test.ts` verifies terminal and startup cleanup retain a workspace marked `git_unpublished_worktree`.                                                                                                                                                                                                                                                                                                                              | [TC-23](e2e/scenarios/23-dirty-unpublished-worktree.md) runs `STUB_SCENARIO=dirty-unpublished-worktree`, confirms a succeeded run retains its publication marker and that both workspace files remain after the issue reaches `Done`.                                                                                                                                                                                                                                                                                                                                                                           |
...
| Controlled Codex startup failures                                            | `test/e2e/claude/claude-docker.spec.ts` starts the built worker through the Codex route without `PROJECT_ID` or `CODEX_PROJECT_ID`, then asserts its final failed heartbeat and exit code `1`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | `pnpm e2e:claude` executes the same built-worker scenario in the Docker-isolated Claude E2E service.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| Agent child isolation and host Git transport                                 | `packages/runtime-codex/src/runtime.test.ts`, `packages/runtime-claude/src/{adapter,mcp-compose}.test.ts`, `packages/tracker-{github,linear}` suites, and `packages/worker/src/git-transport.test.ts` cover unconditional raw/broker-secret stripping, isolated HOME/GH config, disabled subprocess MCP, and real bare-remote fetch/push.                                                                                                                                                                                                                                                                                                                    | [TC-20](e2e/scenarios/20-agent-child-isolation.md) runs `pnpm e2e:claude`; the worker child reports no GitHub, Linear, or broker credential, no inherited Git helper, and only the host HTTP MCP endpoint, while host tool mutations and the post-run assigned-branch push still succeed. Real Claude and Codex worker lifecycles also exercise a non-fast-forward transport failure and assert a failed terminal heartbeat plus non-zero process exit.                                                                                                                                                         |
