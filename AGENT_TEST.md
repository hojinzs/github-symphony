...
- **Docker preflight**: `e2e/run-e2e.sh` and `pnpm e2e:claude` verify that `docker compose` resolves in the current `HOME`/`DOCKER_CONFIG` environment and that the daemon is reachable before creating a project. An unavailable prerequisite exits with status `69`, distinct from Docker's status `125` and from scenario failures.
