...
| Foreign-branch workspace recovery                                            | `packages/orchestrator/src/service.test.ts` creates both clean and dirty workspaces on another issue's branch, verifies their commits/files and branches remain intact, and proves dispatch uses the stable fresh path and assigned issue branch before worker identity preflight; `packages/core/src/observability/event-formatter.test.ts` verifies the retained dirty path and branch are visible.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | `./e2e/run-standalone-project-e2e.sh` pre-populates project alpha's persisted workspace with clean committed work on `fix/2-foreign`, dispatches issue #101 through the real worker identity preflight from the stable recovery path, and verifies the original branch, commit, file, and clean status remain unchanged. `STUB_SCENARIO=recovery-fail` separately exercises repeated dirty recovery; inspect `events.ndjson` for `recovery-dirty-workspace` and confirm retries use the stable recovery path without identity-preflight failures.                                                               |
...
| Workflow reload revision and copy identity signal                            | `packages/core/src/workflow-loader.test.ts` proves the applied revision is short, content-derived, and non-secret; `packages/core/src/observability/snapshot-builder.test.ts` proves snapshots expose applied and source identities; `packages/orchestrator/src/workflow-source-identity.test.ts` covers symlink, committed-ref comparison, synchronized copy (including CRLF normalization), full-history stale copy, independent policy, no-repository-policy classification, and rejects cwd as evidence for a remote repository; `packages/orchestrator/src/service.test.ts` proves HTTPS-clone projects reuse persisted local issue checkouts, stale copies warn, and repositories without committed policy do not warn; `packages/cli/src/commands/doctor.test.ts` proves the same persisted-checkout fallback works from an unrelated cwd and covers relative local paths, unavailable-comparison warnings, and informational no-repository-policy results. | [TC-25](e2e/scenarios/25-workflow-copy-identity.md) advances the local seed repository after copying its committed workflow into a project directory, then verifies packaged status and doctor output expose distinct `sha256:<12 hex>` revisions and a diverged-copy warning. The normal happy-path run also verifies `workflow.revision` matches its `run-dispatched` event.                                                                                                                                                                                                                                  |
...
| Hook-owned repository population and configuration faults                    | `packages/cli/src/workflow/default-hooks.test.ts` verifies `workflow init` writes an executable population hook, then executes it against a real bare fixture to prove a non-promisor clone, assigned-branch checkout, and fetchability from an external bare host repository; `packages/orchestrator/src/git.test.ts` verifies the core creates only directories and preserves reused contents; `packages/orchestrator/src/service.test.ts` covers fresh-hook cleanup, one-shot reuse, and missing-hook degradation without changing an issue retry count; `packages/cli/src/commands/{start,doctor}.test.ts` cover absent and non-executable paths.                                                                                                                                                                                                                                                                                                              | `./e2e/run-standalone-project-e2e.sh` first rejects a packaged standalone project with a missing hook, then dispatches two valid projects whose installed `after_create` hooks populate serviceable workspaces. It commits a worker blob in the hook-created beta workspace, performs the host-equivalent fetch into an external bare repository, and reads the fetched blob. A filtered-clone control missing the superseded policy blob must fail with the `lazy fetching disabled` / `bad pack header` signature.                                                                                            |
