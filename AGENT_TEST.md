...
- **File Tracker** (`@gh-symphony/tracker-file`): reads issues from a JSON file without the GitHub API; `e2e/run-e2e.sh` stages fixture replacements in the mounted directory and renames them atomically so polling cannot observe a partial JSON document. The adapter still retries one transient JSON syntax failure and surfaces persistent malformed JSON.
