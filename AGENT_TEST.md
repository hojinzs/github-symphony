...
| Restart failure isolation                                                    | `packages/orchestrator/src/service.test.ts` seeds a due retrying run whose restart checkout fails and verifies the failed run/project diagnostics while a healthy later candidate dispatches within the same tick.                                                                                                                                                                                                                                                                                                                          | TC-15 records the Docker setup, single-refresh action, and checks for the failed restart diagnostics plus same-tick healthy dispatch.                                                                                  |
