...
| Isolated child Docker CLI plugin access                                      | `packages/core/src/runtime/child-home.test.ts` verifies plugin links resolve into the private child Docker config without copying host `config.json`, and verifies a missing host plugin directory is a no-op; Codex and Claude runtime tests verify their spawned child environments receive the runtime-owned `DOCKER_CONFIG` while direct operator values remain excluded.                                                                                                                                                                                                                                                                                | `./e2e/run-e2e.sh happy 60` crosses the real worker child boundary and confirms the user-level Compose plugin is discoverable from the isolated runtime home; hosts without a plugin retain the Docker preflight failure introduced by #840.                                                                                                                                                                                                                                                                                                                                                                                                           |
