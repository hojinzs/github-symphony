...
| Hook-owned repository population                                             | `packages/cli/src/workflow/default-hooks.test.ts` executes the shipped hook against a real bare fixture and verifies clone plus assigned-branch checkout; `packages/orchestrator/src/git.test.ts` verifies the core creates only directories and preserves reused contents; `packages/orchestrator/src/service.test.ts` covers fresh-hook cleanup and one-shot reuse.                                                                                                                                                                                                                                           | `./e2e/run-standalone-project-e2e.sh` exercises a packaged project dispatch whose generated `after_create` hook populates the workspace before the stub worker starts.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
