...
| Foreign-branch workspace recovery                                            | `packages/orchestrator/src/service.test.ts` creates both clean and dirty workspaces on another issue's branch, verifies their commits/files and branches remain intact, and proves dispatch uses the stable fresh path and assigned issue branch before worker identity preflight; `packages/core/src/observability/event-formatter.test.ts` verifies the retained dirty path and branch are visible.                                                                                                                                                                                                                                                                                      | `STUB_SCENARIO=recovery-fail` exercises repeated dirty recovery through the real worker startup boundary; inspect `events.ndjson` for `recovery-dirty-workspace` and confirm the retained workspace is unchanged while retries use the stable recovery path without identity-preflight failures. Clean foreign-branch recovery is deterministic and unit-isolated because the Docker fixture does not pre-populate an issue workspace with committed foreign work.                                                                                                                                              |
