...
| Agent child isolation and host Git transport                                 | `packages/runtime-codex/src/runtime.test.ts`, `packages/runtime-claude/src/{adapter,mcp-compose}.test.ts`, `packages/core/src/runtime/custom-child-env.test.ts`, `packages/worker/src/non-codex-runtime.test.ts`, `packages/tracker-{github,linear}` suites, and `packages/worker/src/git-transport.test.ts` cover unconditional raw/broker-secret stripping, isolated HOME/GH config, disabled subprocess MCP, custom-auth forwarding, compatibility mode, and real bare-remote fetch/push.                                                                                                                    | [TC-20](e2e/scenarios/20-agent-child-isolation.md) runs `pnpm e2e:claude`; a real custom child covers both default isolation and the documented compatibility mode, proving its private HOME/GH config and Git-helper boundary while differentiating raw-secret exposure. The Claude child dumps its complete environment so the black-box assertion can inject every name from the exported credential-strip contract, require every exported host-constructed assignment, and reject any leaked sentinel without retyping either declaration. Host tool mutations and the post-run assigned-branch push still succeed. Real Claude and Codex worker lifecycles also exercise a non-fast-forward transport failure and assert a failed terminal heartbeat plus non-zero process exit. |
