...
| Failed worker exit retry classification                                      | `packages/orchestrator/src/service.test.ts` feeds a final `turn/failed` runtime update through the worker stderr channel and verifies failure retry classification, exponential backoff, retained error text, and incremented failure count.                                                                                                                                                                                                                                                                                                                                                                      | `STUB_SCENARIO=fail` confirms the Docker worker failure path exits non-zero and is retried by the orchestrator; inspect `/api/v1/state` for `retryKind: "failure"` and a retained failure diagnostic.                                                                                                                                                |
