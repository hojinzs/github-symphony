...
| Dirty foreign-branch workspace recovery                                      | `packages/orchestrator/src/service.test.ts` creates a dirty workspace on another issue's branch, verifies its file and branch remain intact, and proves recovery dispatch uses the stable fresh path and assigned issue branch; `packages/core/src/observability/event-formatter.test.ts` verifies the retained path and branch are visible.                                                                                                                                                                                                                                                                                                                                               | `STUB_SCENARIO=recovery-fail` exercises repeated dirty recovery through the real worker startup boundary; inspect `events.ndjson` for `recovery-dirty-workspace` and confirm the retained workspace is unchanged while retries use the stable recovery path without identity-preflight failures.                                                                                                                                                                                                                                                                                                                |
