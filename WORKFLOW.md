...
3. For each remaining open linked PR, read `reviewDecision`, reviews (including each review's top-level `body`, `state`, and `submittedAt`), review threads (`prReviewState` query), top-level PR comments, and recent issue comments. Find the most recent issue status comment recording a transition to `In review`; its comment `createdAt` is the handoff boundary for top-level review bodies.
4. If any open linked PR has `CHANGES_REQUESTED`, a non-empty top-level `COMMENTED` review body submitted after the most recent transition to `In review`, an unresolved actionable review thread, a human instruction indicating rework, or a recent `Land` → `Ready` transition recorded as a **Land-return rework**, this `Ready` state means **review rework return** — not a fresh pickup and not drift. A qualifying top-level `COMMENTED` review body triggers rework regardless of author identity and does not require an inline thread; this covers steward feedback posted through a shared worker account. When a latest qualifying human approval exists on the current head, narrow the thread condition to unresolved actionable threads created after that approval (compare the thread's first comment `createdAt` with the approval review's `submittedAt`); a thread created at or before that approval is absorbed by the approval and does not trigger rework. When no qualifying approval exists, any unresolved actionable review thread triggers rework as before.
