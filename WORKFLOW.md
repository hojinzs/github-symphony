...
3. For each remaining open linked PR, read `reviewDecision`, reviews (including each review's top-level `body`, `state`, `submittedAt`, and author `__typename`), review threads (`prReviewState` query), top-level PR comments, and recent issue comments. Find the most recent issue status comment recording a transition to `In review`; its comment `createdAt` is the handoff boundary for top-level review bodies. If no such status comment exists, no top-level review body qualifies through this condition; use the other rework signals below.
4. If any open linked PR has `CHANGES_REQUESTED`, a non-empty top-level `COMMENTED` review body from a non-`Bot` author that requests changes or reports findings and was submitted after the handoff boundary defined in item 3, an unresolved actionable review thread, a human instruction indicating rework, or a recent `Land` → `Ready` transition recorded as a **Land-return rework**, this `Ready` state means **review rework return** — not a fresh pickup and not drift. A qualifying top-level `COMMENTED` review body triggers rework even when its author login matches the worker account and does not require an inline thread; this covers steward feedback posted through a shared worker account without treating automated review boilerplate or passing review bodies as actionable. Only `COMMENTED` review bodies qualify through this condition; `APPROVED` and `DISMISSED` bodies retain their separate review-state semantics. When a latest qualifying human approval exists on the current head, narrow the thread condition to unresolved actionable threads created after that approval (compare the thread's first comment `createdAt` with the approval review's `submittedAt`); a thread created at or before that approval is absorbed by the approval and does not trigger rework. When no qualifying approval exists, any unresolved actionable review thread triggers rework as before.
