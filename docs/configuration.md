...
| Variable                           | Default                                          | Read by                             | Audience          | Notes                                                                                                                                   |
| ---------------------------------- | ------------------------------------------------ | ----------------------------------- | ----------------- | --------------------------------------------------------------------------------------------------------------------------------------- |
| `SYMPHONY_MAX_NONPRODUCTIVE_TURNS` | `3`                                              | Orchestrator, worker, Codex runtime | User-facing/ops   | Maximum consecutive turns without detected progress before convergence handling. At the threshold, the worker confirms canonical tracker activity so a completed API-side lifecycle transition is preserved. The orchestrator injects this into workers. |
