...
Child-isolation statements in these tables apply to the Codex and Claude
runtimes. `runtime.kind: custom` continues to receive the full worker
environment for compatibility; [#778](https://github.com/hojinzs/github-symphony/issues/778)
tracks an explicit custom-runtime credential and HOME isolation contract.

| Variable                 | Default                                                                                                          | Read by                                                                | Audience                                            | Notes                                                                                                                                                              |
| ------------------------ | ---------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------- | --------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `GITHUB_GRAPHQL_TOKEN`   | unset                                                                                                            | CLI, orchestrator, GitHub tracker, worker host tools and Git transport | User-facing                                         | Token-only GitHub auth source. Requires `repo`, `read:org`, and `project` scopes. Not inherited by Codex or Claude children.                                       |
...
| `LINEAR_API_KEY`         | unset                                                                                                            | CLI, Linear tracker, worker host tools                                 | User-facing for Linear tracker projects             | Required for Linear repo startup. Retained at the host boundary and not inherited by Codex or Claude children.                                                     |
| `LINEAR_AUTHORIZATION`   | unset                                                                                                            | Linear tracker, worker host tools                                      | Advanced                                            | Optional raw Linear authorization value for host-side Linear operations; it takes priority over `LINEAR_API_KEY` and is not inherited by Codex or Claude children. |
...
| Variable                         | Default          | Read by                                 | Audience          | Notes                                                                                                                         |
| -------------------------------- | ---------------- | --------------------------------------- | ----------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| `GITHUB_TOKEN_BROKER_URL`        | unset            | Worker host tools and Git transport     | User-facing/ops   | Broker endpoint for GitHub tokens. The endpoint is host-only for Codex and Claude and is not inherited by those children.     |
| `GITHUB_TOKEN_BROKER_SECRET`     | unset            | Worker host tools and Git transport     | User-facing/ops   | Shared secret sent to the GitHub token broker. It is declared as a tracker secret and removed from Codex and Claude children. |
...
| `SYMPHONY_ASSIGNED_BRANCH`        | orchestrator-captured checkout branch     | Worker host Git transport                     | Internal/injected | Immutable per-run push target captured before the coding-agent child starts. The host refuses to push when the child moves the worktree to another branch. After a successful push, it also checks the assigned worktree: tracked or untracked edits are recorded in a bounded dedicated `unpublishedWorktree` status record (branch, pushed commit, and file lists), while committed transport remains successful and terminal/startup cleanup retains the workspace. |
