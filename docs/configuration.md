...
| `SYMPHONY_READ_TIMEOUT_MS`         | `5000`                                           | Worker                              | Internal/injected | JSON-RPC read timeout for Codex app-server, or maximum wait for first output from Claude print mode. Sourced from `runtime.timeouts.read_timeout_ms` or legacy `codex.read_timeout_ms`.                                                                                                                                                                                                                                                          |
| `SYMPHONY_TURN_TIMEOUT_MS`         | `3600000`                                        | Worker                              | Internal/injected | Per-turn timeout for Codex app-server, or output-silence interval for Claude print mode (reset by each stdout/stderr chunk). Sourced from `runtime.timeouts.turn_timeout_ms` or legacy `codex.turn_timeout_ms`.                                                                                                                                                                                                                                  |
