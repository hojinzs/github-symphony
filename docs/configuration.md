...
record stores only a SHA-256 digest of that project environment snapshot, never
...
| Variable                                                     | Default          | Read by                                 | Audience          | Notes                                                                                                                                                                    |
| ------------------------------------------------------------ | ---------------- | --------------------------------------- | ----------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `GITHUB_TOKEN_BROKER_URL`                                    | unset            | Worker host Git transport               | User-facing/ops   | Broker endpoint retained in Git publication code for later removal. GitHub workers now require a direct token, which takes precedence and must permit repository pushes. |
| `GITHUB_TOKEN_BROKER_SECRET`                                 | unset            | Worker host Git transport               | User-facing/ops   | Shared secret for the retained but currently unreachable Git broker branch. The adapter still declares it so indirect child-secret stripping remains intact.             |
