...
- **Layer priority**: Symphony built-ins (reserved names `github_graphql`/`linear_graphql`, always win) > project `.mcp.json` > global `~/.gh-symphony/mcp.json` > repo `.mcp.json`. Reserved names preserve the built-in tool contract; workflow transitions themselves use the orchestrator transition endpoint and do not depend on these MCP tools.
...
- **Secrets**: literal tokens forbidden; only §6.1 `$VAR` indirection is allowed (enforced by validation). The existing broker path is a temporary repository-local divergence while agent-started MCP subprocesses can read its credentials. The host-side isolation design in [ADR 2026-08-28](../adr/2026-08-28_agent-tool-isolation.md) removes those child-visible credential paths; it does not treat the current broker wiring as satisfying the upstream §10.5 principle.
