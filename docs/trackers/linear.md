...
| Malformed and optional fields               | State-list reads omit only records with recognized required-field validation failures (`id`, `identifier`, or state name), retain up to 50 sanitized `skippedItems` diagnostics, and continue returning valid records. Requested-ID refreshes remain strict and fail on the same malformed data. Authentication, transport, GraphQL, and paging failures remain atomic. Optional labels, assignee, timestamps, priority, and relations normalize to empty collections or `null`. |
