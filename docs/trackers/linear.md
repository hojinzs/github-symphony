...
| State, branch, labels, priority, timestamps | State is the Linear workflow-state name. `branchName` is trimmed and exposed as the adapter-owned checkout target when present. Labels are trimmed, lowercased, and deduplicated. Priority `0` (No priority) becomes `null`; other Linear values are retained. Timestamps parse to canonical RFC 3339/ISO 8601 or `null`.                                                 |
