...
| Malformed and optional fields               | State-list reads omit only records with recognized required-field validation failures (`id`, `identifier`, or state name), retain up to 50 sanitized `skippedItems` diagnostics, expose the complete omitted-record total as `skippedItemCount`, and continue returning valid records. Unexpected normalization failures still reject the list atomically. Requested-ID refreshes remain strict and fail on the same malformed data. A failed supplemental refresh for an active run is isolated by the orchestrator so other valid candidates can still dispatch while that run remains unresolved for the cycle. Authentication, transport, GraphQL, and paging failures remain atomic. Optional labels, assignee, timestamps, priority, and relations normalize to empty collections or `null`; malformed optional blocker identifiers fall back to `null` without discarding the issue. Candidate polling and startup cleanup report the complete omitted total while rendering only bounded identifier and reason samples before processing valid records. |

Pickup-label filtering does not change those semantics: when it allocates a
filtered result, the adapter preserves the list-level `skippedItems` samples,
`skippedItemCount`, and the latest Linear rate-limit metadata. An all-invalid state
list therefore succeeds with no candidates and bounded diagnostics, while an
all-invalid requested-ID refresh rejects.
