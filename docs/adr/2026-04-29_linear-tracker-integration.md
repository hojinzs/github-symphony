...
3. 기존 `OrchestratorTrackerAdapter` 계약을 깨지 않는 선에서 확장하며, `tracker-file` 수준의 격리를 유지한다.
...
| US-4 | 한 워크스페이스가 GitHub project와 Linear team을 동시에 사용한다 (점진 마이그레이션) | tracker 종류가 다른 두 project가 공존 |
...
GitHub 이슈는 `owner/repo#N`으로 본질적으로 repo에 묶이지만 Linear 이슈는 그렇지 않다. 따라서 Linear tracker config는 `repository` 매핑을 명시해야 한다.

- **단일 repo 매핑** (1차 범위): `tracker.settings.repository = "owner/repo"` — 팀의 모든 이슈가 한 repo로 라우팅
- **라벨 기반 라우팅** (후속, Phase 3): `repositoryLabelPrefix = "repo:"` — `repo:owner/foo` 라벨 기반으로 이슈→repo 결정
...
- `repository`는 config에서 주입 (Linear 응답에는 없음)
...
신규 필드 없음. 기존 `settings: Record<string, unknown>`로 충분.
...
- `gs workflow preview ENG-123` 형식 식별자 인식 (정규식 `^[A-Z]+-\d+$` → linear, `owner/repo#N` → github)
...
1. **Phase 1 (MVP, ~60h)**: tracker-linear 어댑터만, approval workflow는 mock — 운영자가 read-only 폴링/상태 동기화 검증
2. **Phase 2 (~60h)**: extension-linear-workflow 추가 — 풀 라이프사이클 (PR 생성 + Linear 코멘트)
3. **Phase 3 (후속)**: 라벨 기반 multi-repo 라우팅, webhook 지원
...
1. **단일 repo 매핑으로 시작 vs 처음부터 라벨 라우팅** — 본 문서는 단일 repo 매핑을 1차 범위로 권장
