...
- Dirty-workspace recovery leaves the existing workspace untouched and emits a warning-level `recovery-dirty-workspace` event containing its path and branch. If the retained branch names another issue, the orchestrator starts the run in a stable fresh recovery workspace so worker identity preflight remains fail-closed without consuming the retry budget. The orchestrator does not attribute, quarantine, rename, delete, or fail the run because of the retained dirty state.
