...
- Project workflow and polling policy load before tracker candidates are fetched. Path-like `after_create` hooks resolve relative to the workflow source and are validated there; absolute hooks are also validated immediately. Relative run/remove hooks resolve from the populated issue workspace, so pre-start checks surface them as deferred and reconciliation validates them before a retained run restarts. A failure found by pre-dispatch or reconciliation validation is a project-level configuration fault rather than an issue retry; an initial workspace-relative hook execution can consume one retry because its populated path does not exist earlier. The scheduler then applies only the normalized `TrackedIssue.dispatchable` gate before loading issue-specific workflow or starting a worker. It does not encode provider assignment, repository-scope, label, or fork rules; each tracker adapter derives those rules and supplies an explainable `dispatchReason` with non-dispatchable candidates.
