...
| Issue Tracker Client | **MUST** — provide the required adapter operations (§11.1); provider-native agent tools are **MAY** (§11.5)                                                                                                         | `packages/tracker-github`, `packages/tracker-linear`, `packages/tracker-file` — contract is core's `OrchestratorTrackerAdapter`                                                        |
...
| Integration   | **MUST** — provide and document a normalized tracker adapter (§11); provider-native tools are **MAY** (§11.5), while GitHub planning/approval behavior is **none (extension)**, retained for this repository's GitHub workflow                                    |
