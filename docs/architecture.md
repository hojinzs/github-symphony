...
- Agent-child launchers share the core `AGENT_VISIBLE_SYMPHONY_CONTEXT_ENVIRONMENT_NAMES` allowlist for non-secret run and repository identity. Codex, Claude, and custom runtimes compose that context consistently, then remove adapter-declared tracker secrets and generic host credential plumbing at the final child boundary. Tracker credential names are owned exclusively by adapter declarations; adding a tracker does not require a core reserved-auth update.
