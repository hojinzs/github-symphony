...
- Project workflow and polling policy load before tracker candidates are fetched. Path-like workflow hooks are validated as executable regular files (following symlinks) relative to the workflow source; an invalid path is a project-level configuration fault, so no candidate is claimed and no issue retry budget changes. The scheduler then applies only the normalized `TrackedIssue.dispatchable` gate before loading issue-specific workflow or starting a worker. It does not encode provider assignment, repository-scope, label, or fork rules; each tracker adapter derives those rules and supplies an explainable `dispatchReason` with non-dispatchable candidates.
