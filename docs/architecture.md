...
In the mandate columns, **none (extension)** identifies repository functionality
outside the upstream spec's scope; unlike **Implementation-defined**, the spec
does not require an implementation to select or document such functionality.
**Documented divergence** identifies behavior that the spec does cover but this
repository intentionally implements differently, with the cited rationale.

| Spec component       | Spec mandate                                                                                                                                                                                                        | Implementation                                                                                                                                                                            |
| -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Workflow Loader      | **MUST** — discover, parse, validate, and reload `WORKFLOW.md` (§§5.1–5.2, 5.5, 6.2)                                                                                                                                | `packages/core` (`workflow/parser`, `workflow/loader`)                                                                                                                                    |
| Config Layer         | **MUST** — validate the effective configuration before dispatch (§6.3)                                                                                                                                              | `packages/core` (`workflow/parser` resolves per-field runtime/legacy timeout precedence, `workflow/config` exposes effective values), `packages/cli` (`config.ts`, global/project config) |
| Issue Tracker Client | **MUST** — provide the required adapter operations (§11.1); provider-native agent tools are **MAY** by inference because §11.5 does not require first-class orchestrator write APIs (no RFC 2119 keyword)           | `packages/tracker-github`, `packages/tracker-linear`, `packages/tracker-file` — contract is core's `OrchestratorTrackerAdapter`                                                           |
...
| Agent Runner         | **MUST** — launch and drive the targeted Codex app-server protocol in the issue workspace (§§10.1–10.3); Claude print-mode support is **none (extension)**, retained to support an additional coding-agent provider | `packages/worker` + `packages/runtime-codex` / `packages/runtime-claude`                                                                                                                  |
...
| Layer         | Spec mandate                                                                                                                                                                                                                                                                                                                                                                 |
| ------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Policy        | **Implementation-defined** — the repo-defined `WORKFLOW.md` prompt and team-specific ticket handling, validation, and handoff rules are the Policy Layer (§3.2); selected policy still **MUST** be documented where the spec leaves a choice (Normative Language)                                                                                                            |
| Configuration | **MUST** — parse, resolve, validate, and dynamically re-apply workflow configuration (§§5–6); tracker credentials **SHOULD NOT** reach agent children (§3.3), and adapters **MUST** declare authentication environment names (§5.3)                                                                                                                                          |
| Coordination  | **MUST** — apply the orchestration, polling, scheduling, reconciliation, and recovery contracts (§§7–8, 14); scheduler-state persistence and run finalization are **documented divergences** from §§14.3 and 8.6, retained for restart safety and bounded recovery                                                                                                           |
| Execution     | **MUST** — enforce workspace safety (§9) and, for Codex, the targeted app-server launch/turn contract (§10); Claude print-mode, workspace population, and skill injection are respectively **none (extension)**, **MAY** (§9.3), and **none (extension)**, retained to support an additional provider, implementation-owned population, and repository-defined agent tooling |
| Integration   | **MUST** — provide and document a normalized tracker adapter (§11); provider-native tools are **MAY** by inference because §11.5 does not require first-class orchestrator write APIs (no RFC 2119 keyword), while GitHub planning/approval behavior is **none (extension)**, retained for this repository's GitHub workflow                                                 |
...
- Project workflow and polling policy load before tracker candidates are fetched. When workflow-hook execution is explicitly trusted, path-like `after_create` hooks resolve relative to the workflow source and are validated there; absolute hooks are also validated immediately. Relative run/remove hooks resolve from the populated issue workspace, so pre-start checks surface them as deferred and reconciliation validates them before a retained run restarts. A failure found by pre-dispatch or reconciliation validation is a project-level configuration fault rather than an issue retry; an initial workspace-relative hook execution can consume one retry because its populated path does not exist earlier. Without hook trust, enforcing validation is skipped because execution is a no-op, while `doctor` remains available as an advisory diagnostic. The scheduler then applies only the normalized `TrackedIssue.dispatchable` gate before loading issue-specific workflow or starting a worker. It does not encode provider assignment, repository-scope, label, or fork rules; each tracker adapter derives those rules and supplies an explainable `dispatchReason` with non-dispatchable candidates.
- Initial prompt rendering receives the execution phase derived from the configured planning and active states.
- Confirmed tracker transitions outside the configured active states are recorded on the active run with both their timestamp and independently persisted confirmed state; polling may update the run's current `issueState` but cannot rewrite which transition that run confirmed. When the canonical item becomes non-actionable, reconciliation gives the worker a bounded clean-exit grace only for that confirmed state; successful finalization then re-reads the current canonical state and preserves `succeeded` while it still matches the run's confirmation, even if a redundant exit-time Git publication reports a transport failure after the lifecycle transition. An unavailable final read persists a warning-level `run-finalization-deferred` event with a discriminated cause and defers classification for up to three consecutive reconciliation ticks; the third unknown read enters the existing failure-retry path so the run cannot remain pinned. The tick counter intentionally follows reconciliation opportunities rather than wall-clock time, so adaptive polling can stretch the elapsed grace window by up to the configured 10× poll multiplier while preserving a deterministic provider-read budget. A known active or non-actionable result resets the streak. This failure-retry treatment after a successful worker exit intentionally diverges from the upstream specification's normal-exit continuation retry: the repository prioritizes bounded finalization and eventual suppression when canonical state cannot be established, and retains the tracker cause in retry diagnostics instead of reporting a worker failure. Per-turn `state-read` requests do not reload or rewrite workflow snapshots.
...
- Filesystem state store (`OrchestratorFsStore`), leases: `packages/orchestrator/src/fs-store.ts`.
  ...
  repository-local persistence divergence. Run inventory ownership is exposed
  through the core `OrchestratorStateStore`: new callers can query one
  `projectId`, while the legacy global inventory remains available during the
  staged caller migration. The filesystem implementation reads the requested
  project's encoded `projects/<project-id>/runs` directory plus compatible
  records from the legacy flat `runs` directory, filters authoritative
  `run.json` contents by project identity, and prefers the scoped record when a
  run exists in both layouts. Run-record inventory I/O is capped at eight
  concurrent reads for both APIs. This compatibility lookup does not migrate,
  rewrite, or delete legacy records; JSON remains the authoritative storage
  format.
- `OrchestratorService` owns run-query freshness during reconciliation. Each
  ...
  cleanup consumes that scoped evidence snapshot, falls back to one
  project-scoped inventory when no snapshot is supplied, and re-reads only
  live-run candidates before applying identity-checked process liveness. The
  post-suppression refresh
...
- The authenticated `/api/v1/assigned-branch/publish` action authorizes the current run and invokes the worker-owned Git transport from the orchestrator host, allowing the agent to publish before pull-request creation; worker exit invokes the same transport as a backstop. Once a worker has confirmed its issue's current terminal state, active-run reconciliation records success and skips its redundant host publication; terminal cleanup is deferred while that worker process remains live so exit transport cannot lose its cwd. Spawn failures fall back from empty stderr to the process error so transport diagnostics always state a concrete reason. A successful assigned-branch push is not a complete-publication claim when the worktree still has tracked or untracked changes: the worker records a bounded, dedicated `unpublishedWorktree` publication outcome with the pushed branch/commit and file lists while preserving successful transport status. Terminal and startup cleanup retain a workspace carrying either this outcome or a Git transport failure until recovery. This retention is an intentional repository-level divergence from upstream Symphony §8.6: unrecoverable unpublished agent work must not be destroyed by otherwise unconditional terminal cleanup.
- Before worker startup, the orchestrator inspects every persisted checkout's current branch. When either a clean or dirty checkout names another issue, it leaves that checkout untouched and persists a stable fresh recovery workspace as the issue's active workspace for this and later dispatches, so worker identity preflight remains fail-closed without returning to the foreign branch or risking committed user work. Routing the same `workspaceKey` to a stable `<workspaceKey>-recovery` path is an intentional repository-local divergence from the upstream §§9.1–9.2 per-issue path and reuse contract: preserving a foreign-issue checkout takes precedence over reusing its configured path. Dirty-workspace recovery additionally emits a warning-level `recovery-dirty-workspace` event containing the retained path and branch. If the orchestrator-owned recovery checkout is later left on another issue's branch, it is repopulated in place; the operator-owned original remains untouched and recovery paths never chain. Dirty recovery inspection follows the persisted run workspace, while project status keeps warning about the original path and branch until it is clean. The prompt distinguishes that operator-visible retained workspace from the active checkout. The orchestrator does not attribute, quarantine, rename, delete, or fail the run because of the retained original's dirty state. Terminal cleanup removes the persisted active recovery workspace; the original dirty workspace deliberately remains operator-owned and requires manual cleanup after its contents are reviewed.
...
- Linear: `packages/tracker-linear`; it derives provider-native assignment eligibility as the normalized `dispatchable` contract, serves adapter-native CLI smoke reads (`listIssues` / `fetchIssueStatesByIds`) for projects, confirms per-turn `state-read` requests from a fresh issue query, and exposes normalized Linear `branchName` values for dirty-workspace attribution without treating them as checkout refs. State-list normalization isolates recognized malformed-record failures with bounded `skippedItems` samples plus an exact `skippedItemCount`, while unexpected normalization failures and requested-ID refresh, transport, authentication, and pagination remain atomic; list metadata survives pickup-label filtering. The coordination layer treats a failed supplemental active-run refresh as cycle-local and continues dispatching valid list candidates without weakening the adapter's strict refresh contract. Candidate polling and startup cleanup report the exact omitted total and bound rendered identifier/reason samples. Optional malformed blocker identifiers fall back to `null` without hiding an otherwise valid issue. Its pickup labels still filter label-ineligible candidates from the list before dispatch, so they are not retained for explain surfaces as `dispatchable: false` records. This adapter-side label filtering is a repository-level divergence from the upstream scheduler-owned label boundary and differs from the GitHub adapter's retained, reason-bearing records. Linear transition requests are explicitly rejected because state mutation remains worker-owned through `linear_graphql`.
...
| §17.3 empty tracker lookup and malformed refresh | **MUST** — Core Conformance tests are required (§17 introduction; §17.3)                                                                                                                                                         | `packages/tracker-{github,linear,file}/src/*test.ts` assert empty state/ID lookups make no provider call. GitHub and Linear suites assert that malformed requested records fail and that malformed state-list records are omitted with diagnostics; Linear additionally covers mixed and all-invalid lists, bounded diagnostics, unexpected-normalization and paging atomicity, optional-field fallback, metadata preservation through pickup-label filtering, and startup-cleanup diagnostic emission. `packages/orchestrator/src/service.test.ts` verifies a failed strict refresh for an omitted active-run record does not suppress dispatch of valid list candidates.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
