...
| Workspace Manager    | `packages/orchestrator` (`service.ts` directory lifecycle and hook dispatch, `git.ts` safety/branch helpers, `skills.ts` skill injection); project `after_create` hooks own population |
