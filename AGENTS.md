...
3. **Write TCs and run tests**: After completing work, write simple test cases and run the tests to verify. Unit tests (`pnpm test`) are mandatory; when integration behavior needs verification, run black-box tests in the Docker E2E environment. See [AGENT_TEST.md](AGENT_TEST.md) for the concrete testing procedure.
4. **Update living documents**: When behavior, the CLI surface, or the package structure changes, update the related living documents in the same change — user-facing commands and usage in `README.md` and `packages/cli/README.md`, layer/package mapping in the matching slice of `docs/architecture.md`, configuration and environment variables in `docs/configuration.md`, and new E2E scenarios in the scenario table of `AGENT_TEST.md`. Document placement follows [docs/README.md](docs/README.md).
